...
| 标量核刚写过 32 KB 后再向量 load | +913 | L1D 脏行被 L2 探测，约 3.6 拍/行 |
| 首次运行某内核 | +70–330 | VI$ 冷缺失 + 上述脏行探测 |
...
微基准（N=4096，`rtl/results/micro-n4096.log`，以稳态为准）：
...
`python3 scripts/compare_rtl.py --logs rtl/results/rtl-n4096.log`（N = 4096，以 `_warm` 为准）：

| kernel | RTL 冷 | RTL 稳态 | C++ 模型 | 误差 | Python 模型 | 误差 |
|---|---|---|---|---|---|---|
| fma_peak | 12343 | 12308 | 12344 | +0.3% | 12343 | +0.3% |
| dgemm_opt | 8571 | 8407 | 8253 | −1.8% | 8252 | −1.8% |
| sfilter | 5848 | 5571 | 5580 | +0.2% | 5526 | −0.8% |
| gather | 10016 | 9093 | 8357 | −8.1% | 8353 | −8.1% |
| saxpy | 3916 | 3711 | 3241 | −12.7% | 3240 | −12.7% |
| csaxpy | 4576 | 4349 | 3755 | −13.7% | 3754 | −13.7% |
| vvadd | 7648 | 7127 | 6197 | −13.0% | 6196 | −13.1% |
| daxpy | 7214 | 7267 | 6197 | −14.7% | 6196 | −14.7% |

平均绝对误差 8.1%，最大 14.7%。分成两组：

- **计算受限或以 load 为主**（fma_peak、dgemm_opt、sfilter）：误差 ≤ 2%。sfilter 有 3 个 load 与 1 个 store，仍然吻合，说明 store 比例低时混合开销不明显。
- **每块 2 个 load + 1 个 store 的流式内核**（vvadd、daxpy、saxpy、csaxpy）：模型一致地快 13%–15%。以 vvadd 为例，每块 2048 个 load beat + 1024 个 store beat，RTL 每块约 3563 拍，模型约 3100 拍，多出的约 460 拍相当于每个 store beat 在混合流中多花约 0.45 拍，与微基准 copy/ldst 的 −8% 到 −11% 属同一现象且更严重。gather 的 −8% 也含 1 个 store。

因此当前模型的主要系统性偏差只有一个：**与 load 混合时 store 的实际代价高于每 beat 1 拍**。这一效应在 RTL 上还随数据/代码布局波动，尚未找到确定的机理（候选：InclusiveCache 处理 Put 的路径、标量核验证循环把结果数组拉回 L1D 后 Put 需要探测失效）。在此之前，用 `--set store_beat_cycles=1.4` 可以把这四个内核拉到 ±3% 以内，但会让纯 store 与 sfilter 偏慢；`rtl/main.c` 已加入第三次计时（两次运行之间没有标量验证循环）以区分 L1D 探测假设。
